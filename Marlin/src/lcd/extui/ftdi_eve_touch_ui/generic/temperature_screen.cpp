...
void TemperatureScreen::onRedraw(draw_mode_t what) {
  ...
  #endif
  w.increments();
	w.two_buttons(40, GET_TEXT_F(MSG_T_PRESET1_END),41, GET_TEXT_F(MSG_T_PRESET1_BED));
	w.two_buttons(42, GET_TEXT_F(MSG_T_PRESET2_END),43, GET_TEXT_F(MSG_T_PRESET2_BED));
...
bool TemperatureScreen::onTouchHeld(uint8_t tag) {
  ...
  switch (tag) {
    ...
    case 23: UI_INCREMENT(TargetTemp_celsius, CHAMBER); break;
		case 40: setTargetTemp_celsius(getPresetEndTemp1(), E0); break;
		case 41: setTargetTemp_celsius(getPresetBedTemp1(), BED); break;
		case 42: setTargetTemp_celsius(getPresetEndTemp2(), E0); break;
		case 43: setTargetTemp_celsius(getPresetBedTemp2(), BED); break;
    #ifndef NO_TOOLHEAD_HEATER_GCODE
